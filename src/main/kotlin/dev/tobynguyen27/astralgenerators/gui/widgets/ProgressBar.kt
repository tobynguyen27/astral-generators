...
class ProgressBar(bg: ResourceLocation, bar: ResourceLocation,  field: Int,  maxField: Int): WBar(
    bg, bar, field, maxField, Direction.RIGHT
) {
    override fun addTooltip(information: TooltipBuilder) {
        val current = properties.get(field)
        val max = properties.get(max)

        information.add(TranslatableComponent(Texts.PROGRESS).withStyle(ChatFormatting.GRAY).append(
            TextComponent(" ${calculateFormattedPercentage(current, max, "0")}%")
        ))
    }
}
